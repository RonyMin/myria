...
public final class SQLiteAccessMethod extends AccessMethod {
  ...
  public void createIndexes(final RelationKey relationKey, final Schema schema, final List<List<IndexRef>> indexes)
     ...
     */
    final String indexProgramName = new StringBuilder(relationKey.getProgramName()).append("__myria_indexes_").append(
    ...
    for (List<IndexRef> index : indexes) {
      ...
      statement.append("CREATE INDEX ").append(indexName).append(" ON ").append(tempTableName).append(columns
          .toString());
